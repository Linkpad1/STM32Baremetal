...
void interrupt_setup(void)
{
	...
	//SYSCFG->EXTICR[0]  |= (((uint32_t)6) << 0);
	//SYSCFG->EXTICR[0]  |= (((uint32_t)6) << 4);
    //SYSCFG->EXTICR[0] |= 6; // Tie PG to EXTI0
	//SYSCFG->EXTICR[0] |= (6 << 7); // Tie PG to EXTI1
	...
	//NVIC->ISPR[EXTI0_IRQn] = 0x10; // Priority level 1
	//NVIC->ISPR[EXTI1_IRQn] = 0x20; // Priority level 2
